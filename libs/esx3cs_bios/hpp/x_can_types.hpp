//-----------------------------------------------------------------------------
/*!
   \file        x_can_types.hpp
   \brief       CPP namespace extention for x_can_types.h
...
#ifndef __X_CAN_TYPES_HPP
#define __X_CAN_TYPES_HPP
...
typedef struct
{
   stw_types::uint32       u32_Id;        //!< CAN identifier 11/29bit
   stw_types::uint8        u8_Xtd;        //!< bXtd == true: 29bit ID, bXtd == false: 11bit ID
} T_x_can_id;
...
typedef struct
{
   ...
   stw_types::uint32       u32_Time;       //!< RX time stamp (not used for TX messages)
   stw_types::uint8        au8_Data[8];    //!< message data 8x stw_types::uint8 (aligned to 32bit boundary)
...
*/
//-----------------------------------------------------------------------------
typedef void (*PR_x_can_bus_FailureCallback)(const stw_types::uint32 ou32_CallbackParameter, const stw_types::uint16 ou16_Channel,
                                             const stw_types::uint16 ou16_BusState);
...
*/
//-----------------------------------------------------------------------------
typedef stw_types::sint16 (*PR_x_can_obj_InterruptCallback)(const stw_types::uint32 ou32_CallbackParameter, const stw_types::uint16 ou16_Channel,
                                                            const void * const opv_Handle, T_x_can_msg * const opt_CanMsg,
                                                            const stw_types::uint16 ou16_Direction);
...
typedef struct
{
   ...
   volatile stw_types::uint32   u32_LastErrorTime;   //!< system time when last error occurred
   volatile stw_types::uint16   u16_BusState;        //!< bus status (bit coded)
}T_x_can_bus_status;
...
typedef struct
{
   ...
   stw_types::uint8                     u8_MsgType;              //!< X_CAN_OBJ_TYPE_RX, X_CAN_OBJ_TYPE_TX, X_CAN_OBJ_TYPE_TX_RTR
   stw_types::uint16                    u16_Pause;               //!< TX pause in steps of 0.5ms
   stw_types::uint16                    u16_SoftBuffer;          //!< CAN software FIFO size
   stw_types::uint16                    u16_HardBuffer;          //!< CAN hardware FIFO size
   stw_types::uint32                    u32_CallbackParameter;   //!< callback parameter
...
typedef struct
{
   ...
   volatile stw_types::uint32    u32_LastInterruptTime;   //!< last RX/TX event
   stw_types::uint16             u16_NumberMsgs;          //!< number of messages in FIFO buffer
   stw_types::uint8              u8_Usage;                //!< X_CAN_OBJ_LOCKED, X_CAN_OBJ_IN_USE, X_CAN_OBJ_NO_INIT
   volatile stw_types::uint8     u8_MsgLostFlag;          //!< at least one RX message was lost by hardware
   volatile stw_types::uint8     u8_OverflowCount;        //!< number of lost messages, will be reset by reading it out
}T_x_can_obj_status;
...
typedef struct
{
   stw_types::uintn   q_CopyDlc    :  1;                   //!< copy DLC to destination CAN bus
   stw_types::uintn   q_CopyId     :  1;                   //!< copy ID to destination CAN bus
   stw_types::uintn   q_CopyData   :  1;                   //!< copy data to destination CAN bus
   stw_types::uintn   q_AutoTX     :  1;                   //!< automatic transmission of destination data
   stw_types::uintn   q_AllIdType  :  1;                   //!< handle standard 11bit and extended 29bit IDs
   stw_types::uintn   un_RTRMode   :  2;                   //!< RTR mode
...
typedef struct
{
   ...
   stw_types::uint16            u16_HardBuffer;            // gateway hardware FIFO size (0 = no FIFO)
   ...
   stw_types::uint32            u32_CallbackParameter;     // callback parameter (C++)
   PR_x_can_obj_InterruptCallback pr_Callback;  // TX/RX interrupt callback
} T_x_can_gateway_config;


#endif  // __X_CAN_TYPES_HPP
